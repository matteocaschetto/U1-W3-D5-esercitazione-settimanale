...
function deleteOne(str1, valBooleano) {
  if (valBooleano) {
    return str1.slice(1);
  } else {
...
console.log(deleteOne("Matteo", true));
console.log(deleteOne("MAtteo", false));
...
console.log(onlyLetters("Io ho 4 cani e 3 gatti"));
...
console.log(isThisAnEmail("matteogmail.it"));
...
console.log(isTodayMyBirthday(18, 10));
...
function newestMovie(movies) {
  let lastMov = movies[0];

  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Year > lastMov.Year) {
      lastMov = movies[i];
    }
  }
...
function onlyInLastMillennium(movies) {
  ...
  for (let i = 0; i < movies.length; i++) {
    if (parseInt(movies[i].Year) >= 1991 && parseInt(movies[i].Year) < 2001) {
...
function searchByTitle(searchStr) {
  ...
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.includes(searchStr)) {
...
*/
function searchAndDivide(searchStr) {
  const result = {
    match: [],
    unmatch: [],
  };
...
function printTd() {
  ...
  for (let i = 0; i < tableCells.length; i++) {
    console.log(tableCells[i].innerText);
  }
}
printTd();
